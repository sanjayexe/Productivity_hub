...
const CalendarPage = () => {
    ...
    const addEvent = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost:5000/api/events', { title, description, start, end, allDay }, config);
            setTitle('');
            setDescription('');
            setAllDay(false);
            fetchEvents();
        ...
        <div className="animate-fade-in">
             <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <button onClick={prevMonth} className="btn-primary" style={{ padding: '0.5rem 1rem' }}>&lt;</button>
                    <h2 style={{ margin: 0, minWidth: '200px', textAlign: 'center' }}>
                        {monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}
                    </h2>
                    <button onClick={nextMonth} className="btn-primary" style={{ padding: '0.5rem 1rem' }}>&gt;</button>
                </div>
            </div>
                        ...
                        {[...Array(days)].map((_, i) => {
                            ...
                            return (
                                <div 
                                    ...
                                    onMouseLeave={(e) => e.currentTarget.style.background = isToday ? 'rgba(99, 102, 241, 0.1)' : 'transparent'}
                                >
                                    <div style={{ fontWeight: 'bold', marginBottom: '0.25rem', color: isToday ? 'var(--primary-color)' : 'inherit' }}>{day}</div>
                    ...
                    <div className="card">
                        ...
                        <div style={{ marginBottom: '1.5rem', maxHeight: '200px', overflowY: 'auto' }}>
                             {getEventsForDay(selectedDate.getDate()).length === 0 ? (
                                <p style={{ color: 'var(--text-secondary)' }}>No events for this day.</p>
                             ) : (
                                 getEventsForDay(selectedDate.getDate()).map(ev => (
                                     <div key={ev._id} style={{ marginBottom: '0.5rem', padding: '0.5rem', background: 'rgba(255,255,255,0.05)', borderRadius: '4px' }}>
