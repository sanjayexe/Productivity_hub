...
const fetchMotivationalQuotes = async () => {
  try {
    ...
    const prompt = `Generate 5 short, inspiring motivational quotes for productivity. 
...
Do NOT wrap the json in backticks, code blocks, or markdown.`;
    ...
    const quotes = data.map(q => ({ text: q.text, author: q.author || "Unknown" }));

    return {
      fetchedAt: new Date().toISOString(),
      quotes: quotes.length ? quotes : [{ text: "Every day is a new opportunity to grow.", author: "Unknown" }]
    };
  } catch (error) {
...
const getRandomQuote = async () => {
  ...
  try {
    if (!shouldRefreshQuotes() && fs.existsSync(quotesFile)) {
      ...
      fs.writeFileSync(quotesFile, JSON.stringify(quotesData, null, 2), "utf8");
    }
  } catch (err) {
    console.error("Error in cached quotes:", err);
    quotesData = {
      quotes: [{ text: "Every day is a new opportunity to grow.", author: "Unknown" }]
    };
  }

  // Return a random quote from the cached array
  const randomItem = quotesData.quotes[Math.floor(Math.random() * quotesData.quotes.length)];
  return {
